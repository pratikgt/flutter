...
class AuthRepository implements IAuthRepository {
  ...
  Future<Either<Failure, bool>> register({
    ...
    try {
      final existingUser = await _authDataSource.getUserByEmail(email);
      if (existingUser != null) {
        return const Left(
          LocalDatabaseFailure(message: "Email already registered"),
        );
  ...
  Future<Either<Failure, AuthHiveModel>> login({
    ...
    try {
      final model = await _authDataSource.login(
        email: email,
        password: password,
      );
  ...
  @override
  Future<Either<Failure, AuthHiveModel>> getCurrentUser() async {
    try {
      final model = await _authDataSource.getCurrentUser();
      if (model == null) {
        return const Left(
  ...
  Future<Either<Failure, bool>> logout() async {
    try {
      ...
      if (!result) {
        return const Left(
          LocalDatabaseFailure(message: "Logout failed"),
        );
      }
      return const Right(true);
    } catch (e) {
